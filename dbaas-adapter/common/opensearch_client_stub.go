...
func (cs *ClientStub) roleManipulations(name string, method string) string {
	...
	case http.MethodGet:
		...
		case strings.Contains(name, "refresh"):
    		return `{"dbaas_refresh_role":{"reserved":false,"hidden":false,"cluster_permissions":[],"index_permissions":[{"index_patterns":["${attr.internal.resource_prefix}*"],"fls":[],"masked_fields":[],"allowed_actions":["indices:admin/refresh","INDICES:ADMIN/REFRESH"]}],"tenant_permissions":[],"static":false}}`
		default:
