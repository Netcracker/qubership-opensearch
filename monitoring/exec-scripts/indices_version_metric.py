...
def __configure_logging(log):
    ...
    log_handler.setLevel(
        logging.DEBUG
        if os.getenv("ELASTICSEARCH_MONITORING_SCRIPT_DEBUG")
        else logging.INFO
    )
...
def _get_indices_settings(elasticsearch_url: str):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.warning(
                f"Failed to get indices settings. Status code: {response.status_code}"
            )
            return {}
    except Exception:
...
def _collect_metrics(elasticsearch_url: str):
    ...
    # namespace (from OS_PROJECT) is required for Prometheus alert/dashboard selectors
    namespace = os.getenv("OS_PROJECT", "")
    metrics = []
    for (version_str, version_num), count in version_counts.items():
        metric_line = (
            f'opensearch_indices_version_created,version="{version_str}",version_num={version_num},namespace="{namespace}" '
            f"count={count}i"
        )
...
def run():
    try:
        ...
        elasticsearch_url = (
            f"{elasticsearch_protocol}://{elasticsearch_host}:{elasticsearch_port}"
        )
