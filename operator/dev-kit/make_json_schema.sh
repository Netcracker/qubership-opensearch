...
nc-schema-gen --leveling --skip-type-mismatch --off-array-list-mismatch-log \
  ...
  -v $ROOT_PATH/operator/charts/helm/opensearch-service/values.yaml \
  -s $ROOT_PATH/operator/charts/helm/opensearch-service/values.schema.json \
  -r $ROOT_PATH/operator/charts/helm/opensearch-service/values.rules.json
