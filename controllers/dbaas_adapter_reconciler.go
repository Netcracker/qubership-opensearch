...
func (r DbaasAdapterReconciler) Configure() error {
	if r.cr.Status.DisasterRecoveryStatus.Mode != r.cr.Spec.DisasterRecovery.Mode || r.cr.Status.DisasterRecoveryStatus.Status == "failed" {
		...
		if strings.ToLower(r.cr.Spec.DisasterRecovery.Mode) == "active" {
			...
			err := r.reconciler.scaleDeploymentWithCheck(r.cr.Spec.DbaasAdapter.Name, r.cr.Namespace, 1, waitingInterval, scaleTimeout, r.logger)
			if err != nil {
				return err
			}
			...
			err := r.reconciler.scaleDeploymentWithCheck(r.cr.Spec.DbaasAdapter.Name, r.cr.Namespace, 0, waitingInterval, scaleTimeout, r.logger)
			if err != nil {
				return err
			}
