...
func (r CuratorReconciler) Configure() error {
	if r.cr.Spec.DisasterRecovery != nil && r.cr.Status.DisasterRecoveryStatus.Mode != "" {
		...
		if strings.ToLower(r.cr.Spec.DisasterRecovery.Mode) == "active" {
			...
			err := r.reconciler.scaleDeploymentForDR(r.cr.Spec.Curator.Name, r.cr.Namespace, 1, r.cr.Spec.DisasterRecovery.NoWait, r.logger)
			if err != nil {
				return err
			}
			...
			err := r.reconciler.scaleDeploymentForDR(r.cr.Spec.Curator.Name, r.cr.Namespace, 0, r.cr.Spec.DisasterRecovery.NoWait, r.logger)
			if err != nil {
				return err
			}
