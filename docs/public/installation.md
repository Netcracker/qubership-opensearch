...
* [Best Practices and Recommendations](#best-practices-and-recommendations)
  ...
  * [Index Configurations](#index-configurations)
    * [Global Index Settings by Pattern](#global-index-settings-by-pattern)
...
* [Upgrade](#upgrade)
  ...
  * [Migration](#migration-1)
    ...
    * [Resource Migration](#resource-migration-1)
...
* The custom resource definition `OpenSearchService` should be created/applied before the installation if the corresponding rights cannot be provided to the deployment user.
    ...
    <!-- #GFCFilterMarkerStart# -->  
  The CRD for this version is stored in [crd.yaml](/operator/charts/helm/opensearch-service/crds/crd.yaml) file and can be
...
* To run `privileged` containers or actions on Kubernetes before 1.25 version, apply `PodSecurityPolicy` from [psp.yaml](/operator/charts/helm/opensearch-service/templates/psp.yaml),
...
| `pvc.metadata.annotations` | object | no        | {}            | The annotations added to all OpenSearch persistent volume claims (master, arbiter, data nodes and curator/snapshots). Master, data, and arbiter PVCs annotations (merged with the per-role `opensearch.<master\|data\|arbiter>.persistence.annotations`) are added to StatefulSet `volumeClaimTemplates`: new PVCs get them at creation, and on upgrade the `resource-migrator` recreates any StatefulSet whose desired PVC annotations changed (with `--cascade=orphan`) and patches the existing PVCs to match (requires `resourceMigration.enabled: true`). |
...
| INFRA_RESTRICTED_ENVIRONMENT           | boolean | no        | `false`       | Specifies whether the OpenSearch service is to be deployed in restricted environment.       |
...
| `global.velero.timeout`                      | string  | no        | 600s                                                                       | The time to wait for the command from `pre.hook.backup.velero.io/command` annotation to execute. The hook is considered in error if the command exceeds the timeout.                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `global.externalOpensearch.config`           | object  | no        | See in [values.yaml](/operator/charts/helm/opensearch-service/values.yaml) | The configuration of common properties for external OpenSearch. For more information, see [Configuring OpenSearch](https://opensearch.org/docs/latest/install-and-configure/configuring-opensearch/index/).                                                                                                                                                                                                                                                                                                                                                |
...
| `global.imagePullSecrets`                    | list    | no        | []                                                                         | The list of references to secrets in the same namespace to use for pulling any of the images used by the all containers.                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `global.restrictedEnvironment`               | boolean | no        | false                                                                      | Whether the OpenSearch service is to be deployed in restricted environment. If it is set to `true`, necessary cluster entities (`Cluster Role`, `Cluster Role Binding`, `Pod Security Policy`) are not created automatically.                                                                                                                                                                                                                                                                                                                              |
...
| `global.disasterRecovery.tls.certificates.key`                             | string  | no        | ""                       | The private key in base64 format. It is required if `global.tls.enabled` parameter is set to `true`, `global.tls.generateCerts.certProvider` parameter is set to `dev` and `global.tls.generateCerts.enabled` parameter is set to `false`.                                                                           |
| `global.disasterRecovery.tls.certificates.ca`                              | string  | no        | ""                       | The root CA certificate in base64 format. It is required if `global.tls.enabled` parameter is set to `true`, `global.tls.generateCerts.certProvider` parameter is set to `dev` and `global.tls.generateCerts.enabled` parameter is set to `false`.                                                                   |
...
| `global.disasterRecovery.siteManagerApiGroup`                              | string  | no        | netcracker.com           | The `apiGroup` used for `SiteManager` resource.                                                                                                                                                                                                                                                                      |
...
| `global.disasterRecovery.deleteFollowerIndex`                              | boolean | no        | true                     | Whether the follower index is automatically deleted whenever the corresponding leader index is deleted.                                                                                                                                                                                                              |
...
| `dashboards.ingress.hosts`                           | list    | no        | [{"host": "chart-example.local", "paths": [{"path": "/"}]}]                  | The list of hostnames for the dashboards' ingress.                                                                                                                                                                                                                                                                 |
| `dashboards.ingress.tls`                             | list    | no        | []                                                                           | The list of TLS configurations for the dashboards' ingress.                                                                                                                                                                                                                                                        |
| `dashboards.envoyGateway.enabled`                    | boolean | no        | false                                                                        | Enables creation of a Gateway API HTTPRoute resource to expose OpenSearch Dashboards through Envoy Gateway.                                                                                                                                                                                                        |
| `dashboards.envoyGateway.host`                       | string  | no        | ""                                                                           | Specifies the hostname used in the generated HTTPRoute for accessing OpenSearch Dashboards. If this value is empty, the first entry from `dashboards.ingress.hosts` will be used.                                                                                                                                  |
| `dashboards.envoyGateway.parentRefs`                 | list    | no        | [{"name": "default-external-gateway", "namespace": "gateway-system"}]        | List of Gateway resources that the generated HTTPRoute for OpenSearch Dashboards will attach to.                                                                                                                                                                                                                   |
...
| `operator.logLevel`                  | string  | no        | info                     | The log level for the OpenSearch Service Operator. Set to `debug` to enable debug logging.                                                                                                                                                                                                                      |
...
| `opensearch.securityConfig.ldap.search.usernameAttribute`     | string  | no        | ""                                                                         | Name of the attribute used to look for the username in the directory entry. If set to null, the DN is used.                                                                                                                                                                                                            |
...
| `opensearch.client.envoyGateway.enabled`               | boolean | no        | false                                                                                                       | Enables creation of a Gateway API HTTPRoute resource to expose the OpenSearch client through Envoy Gateway.                                                                                                                                                                                                                |
| `opensearch.client.envoyGateway.host`                  | string  | no        | ""                                                                                                          | Specifies the hostname used in the generated HTTPRoute for accessing the OpenSearch client API. If this value is empty, the first entry from `opensearch.client.ingress.hosts` will be used.                                                                                                                               |
| `opensearch.client.envoyGateway.parentRefs`            | list    | no        | [{"name": "default-external-gateway", "namespace": "gateway-system"}]                                       | List of Gateway resources that the generated HTTPRoute for OpenSearch client will attach to.                                                                                                                                                                                                                               |
...
| `opensearch.snapshots.s3.sslSecretName`      | string  | no        | `""`          | This parameter specifies name of the secret with CA certificate for S3 connections. If secret not exists and parameter `opensearch.snapshots.s3.sslCert` is specified secret will be created, else boto3 certificates will be used.                                                                                                                                     |
| `opensearch.snapshots.s3.sslCert`            | string  | no        | `""`          | The root CA certificate in base64 format. It is required if pre-created secret with certificates not exists and default boto3 certificates will not be used.                                                                                                                                                                                                            |
...
| `opensearch.snapshots.s3Aliases`             | list    | no        | []            | The list of S3 backup alias definitions. If empty, aliases secret is not rendered and aliases are not mounted to curator.                                                                                                                                                                                                                                               |
...
| `dbaasAdapter.registrationEnabled`                              | boolean | no        | false                                                  | Using the registrationEnabled parameter we can determine whether registration is enabled                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `dbaasAdapter.prefixUniqueEnabled`                              | boolean | no        | true                                                   | Using the prefixUniqueEnabled parameter we can determine whether resource prefix intersection validation is enabled                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `curator.storage.storageClass`                             | string  | no        | ""                       | The storage class name for the curator snapshot PVC. If empty, the value of `opensearch.master.persistence.storageClass` is used as a fallback. At least one of these two parameters must be specified when `curator.enabled` is `true`.                                                                                                                                                                                                                                                                                                                                        |
| `curator.storage.size`                                     | string  | no        | 1Gi                      | The size of the persistent volume for OpenSearch Curator snapshot storage.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `curator.storage.persistentVolumeClaim`                    | string  | no        | ""                       | The name of an existing persistent volume claim to use for curator snapshot storage. If empty, a new PVC named `pvc-<name>-curator` is created automatically.                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| Parameter                                      | Type   | Mandatory | Default value | Description                                                                                   |
|------------------------------------------------|--------|-----------|---------------|-----------------------------------------------------------------------------------------------|
| `updateResourcesJob.resources.requests.cpu`    | string | no        | 75m           | The minimum number of CPUs the update resources container should use.                         |
| `updateResourcesJob.resources.requests.memory` | string | no        | 75Mi          | The minimum amount of memory the update resources container should use.                       |
| `updateResourcesJob.resources.limits.cpu`      | string | no        | 150m          | The maximum number of CPUs the update resources container should use.                         |
| `updateResourcesJob.resources.limits.memory`   | string | no        | 150Mi         | The maximum amount of memory the update resources container should use.                       |
...
| Parameter                                  | Type    | Mandatory | Default value                                    | Description                                                                                                                                                                   |
|--------------------------------------------|---------|-----------|--------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `groupMigration.enabled`                   | boolean | no        | true                                             | Enables the owner reference migration `pre-install`/`pre-upgrade` hook job.                                                                                                   |
| `groupMigration.oldGroupPrefix`            | string  | no        | qubership.org/                                   | The old API group prefix. Owner references whose `apiVersion` starts with this value are removed from the processed resources.                                                |
| `groupMigration.labelSelector`             | string  | no        | ""                                               | The optional label selector that limits the resources processed by the job, for example `app.kubernetes.io/part-of=app-name`. When empty, all target resources are processed. |
| `groupMigration.resources.requests.cpu`    | string  | no        | 20m                                              | The minimum number of CPUs the group migration container should use.                                                                                                          |
| `groupMigration.resources.requests.memory` | string  | no        | 64Mi                                             | The minimum amount of memory the group migration container should use.                                                                                                        |
| `groupMigration.resources.limits.cpu`      | string  | no        | 100m                                             | The maximum number of CPUs the group migration container should use.                                                                                                          |
| `groupMigration.resources.limits.memory`   | string  | no        | 256Mi                                            | The maximum amount of memory the group migration container should use.                                                                                                        |
| `groupMigration.image`                     | string  | no        | ghcr.io/netcracker/qubership-docker-kubectl:main | The image used by the group migration job.                                                                                                                                    |
| `groupMigration.imagePullPolicy`           | string  | no        | Always                                           | The image pull policy for the group migration container.                                                                                                                      |
| `groupMigration.runAsNonRoot`              | boolean | no        | true                                             | If `true`, applies restricted security context (non-root, read-only filesystem, drops all capabilities) to the job pod.                                                       |

## Migration

| Parameter                             | Type    | Mandatory | Default value | Description                                                                                         |
|---------------------------------------|---------|-----------|---------------|-----------------------------------------------------------------------------------------------------|
| `migration.enabled`                   | boolean | no        | false         | Enables the migration `pre-upgrade` job to migrate 1.x indices before or during the upgrade to 3.x. |
| `migration.args`                      | list    | no        | []            | The list of flags passed to the migration tool.                                                     |
| `migration.resources.requests.cpu`    | string  | no        | 100m          | The minimum number of CPUs the migration container should use.                                      |
| `migration.resources.requests.memory` | string  | no        | 256Mi         | The minimum amount of memory the migration container should use.                                    |
| `migration.resources.limits.cpu`      | string  | no        | 500m          | The maximum number of CPUs the migration container should use.                                      |
| `migration.resources.limits.memory`   | string  | no        | 512Mi         | The maximum amount of memory the migration container should use.                                    |
...
| `integrationTests.atpReport.atpStorage.serverUrl`   | string  | no        | `https://s3.amazonaws.com` | S3 API endpoint URL (for example `https://s3.amazonaws.com` or `https://minio.example.com`). |
...
| `integrationTests.environmentName`                  | string  | no        | opensearch                 | Environment label for organizing result paths in S3. |
...
command in [OpenSearch Chart](/operator/charts/helm/opensearch-service):
...
<!-- #GFCFilterMarkerStart# -->
The CRD for this version is stored in [crd.yaml](/operator/charts/helm/opensearch-service/crds/crd.yaml) and can be applied with the following command:
