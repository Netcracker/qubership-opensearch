...
| `pvc.metadata.annotations` | object | no        | {}            | The annotations added to all OpenSearch persistent volume claims (master, arbiter, data nodes, and snapshots PVCs). They are merged with the per-component `*.persistence.annotations`, where the component-specific annotations take precedence on key conflicts. |
