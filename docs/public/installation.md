...
| `pvc.metadata.annotations` | object | no        | {}            | The annotations added to all OpenSearch persistent volume claims (master, arbiter, data nodes, and snapshots). They are merged with the per-role `opensearch.<master\|data\|arbiter>.persistence.annotations`. Master, data, and arbiter node PVCs are created from StatefulSet `volumeClaimTemplates` (an immutable field): new PVCs get them at creation, and on upgrade the `resource-migrator` pre-upgrade hook recreates any StatefulSet whose desired PVC annotations changed (with `--cascade=orphan`, so pods and PVCs are kept) and patches the existing PVCs to match (requires `resourceMigration.enabled: true`). |
